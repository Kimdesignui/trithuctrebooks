...
const reply_comment = {
    ...
    comment: function (customerId, bookId, url) {
        ...
        $.post('/books/comment', { model: obj }, function (data) {
            if (data.Error == true) {
                if (data.ID == "0") {
                    window.location.href = DOM_Purify.sanitize(data.Url);
                }
                else {
...
const checkout = {
    ...
    noti: function (type, message, time, url = '') {
        ...
        if (type == "success") {
            ...
            var myVar = setInterval(function () {
                ...
                if (url !== '')
                    window.location.href = DOM_Purify.sanitize(url);
                clearTimeout(myVar);
            }, time);
        } else if (type == "error") {
    ...
    remove_cart: function (elm, bid, e) {
        $.post("/checkout/removecart", { bid: bid, e: e }, function (count) {
    ...
    voucher: function () {
        ...
        if (code !== '' && code !== undefined && code !== null) {
                ...
                if (data.Type === 0) {
                    ...
                    data.ListPaperId.forEach((i, id) => {
                        ...
                        data.PromotionBonusBooks.forEach((item, index) => {
                            if (item.QueryId === i) {
                                var giftHtml = '<a href="[url]" data-queryid="[id]"> <i class="fas fa-gift mr-2"></i>Tặng kèm: [name][type][unit]</a>'
            ...
            $('.cart-gift-code-item').html('');
            $('.total-discount-voucher').val(0);
            fetch('/cookie/delete/voucher', { method: 'POST' });
            checkout.loading(".section-checkout", "hide");
    ...
    orderDetail: function () {
        ...
        $(document).on('click', '.see-desktop', function () {
            ...
            $.get('/customer/order/' + code, function (data) {
                $('#see-desktop .modal-body').html(DOM_Purify.sanitize(data));
...
const authorCarousel = {
    ...
    authorCarousel: function () {
        var $owl = $(".tac-gia-tieu-bieu .ttb-author-carousel");
        ...
        allItems.forEach((item) =>
            item.addEventListener("click", () => {
                ...
                if (subItems) {
                    ...
                    window.location.href = DOM_Purify.sanitize(link);
                }
            })
        );
    },
...
const cookie = {
    set: function (name, value, days) {
        ...
        document.cookie = DOM_Purify.sanitize(name) + "=" + DOM_Purify.sanitize(value) + expires + "; path=/";
    },
    get: function (name) {
        var nameEQ = DOM_Purify.sanitize(name) + "=";
...
const validatePass = {
    validatePassword: function (ip_pwd) {
        var password = document.getElementById(ip_pwd).value;
        var validationContainer = document.getElementById('passwordValidation');
        // Validation rules
        var rules = {
        ...
        this.updateValidationItem('check-length', rules.length);
        this.updateValidationItem('check-number', rules.number);
        this.updateValidationItem('check-lowercase', rules.lowercase);
        this.updateValidationItem('check-uppercase', rules.uppercase);
        this.updateValidationItem('check-special', rules.special);
        // Show validation container if password has any content
        validationContainer.style.display = password.length > 0 ? 'block' : 'none';
    },
    updateValidationItem: function (elementId, isValid) {
        var element = document.getElementById(elementId);
        var icon = element.querySelector('.validation-icon');
...
/* ============================= 42, Category Filter (book type, sort, price, tracking) ============================= */
const categoryFilter = {
    ...
    // Lọc theo định dạng sách: data-booktype trên <a> hoặc <button>
    bookType: function () {
        document.querySelectorAll('[data-booktype]').forEach(function (el) {
            el.addEventListener('click', function () {
                var href = window.location.href.replace(/&amp;/g, '&');
                const url = new URL(href);
                const params = url.searchParams;
                if (this.dataset.booktype && this.dataset.booktype !== -1) {
                    params.set('type', this.dataset.booktype);
                } else
                    params.delete('type');
                Array.from(params.keys()).forEach(key => {
                    if (!params.get(key)) params.delete(key);
                });
                window.location.href = `${url.pathname}?${DOM_Purify.sanitize(params.toString())}`;
            });
        });
    },

    sortSelect: function (elm) {
        var href = window.location.href.replace(/&amp;/g, '&');
        const url = new URL(href);
        const params = url.searchParams;
        var sort = $(elm).val();
    ...
    // Sắp xếp bằng radio trong .cate-filter_dropdow (mobile)
    sort: function () {
        document.querySelectorAll('input[name="filter"]').forEach(function (radio) {
            radio.addEventListener('change', function () {
                categoryFilter.sortSelect(this);
            });
        });
        var selectSort = document.querySelector('.select-filter select');
        if (selectSort) {
    ...
    // Tìm theo giá: form#form-price-desktop và form#form-price-mobile
    priceForm: function () {
        var form = document.querySelector('.box-price_form');
        if (form) {
            form.addEventListener('submit', function (e) {
                ...
                else params.delete('fromPrice');
                if (toPrice) params.set('toPrice', toPrice);
                else params.delete('toPrice');
                Array.from(params.keys()).forEach(key => {
                    if (!params.get(key)) params.delete(key);
                });
                window.location.href = `${url.pathname}?${DOM_Purify.sanitize(params.toString())}`;
    ...
    // Tracking mousedown trên link sách: a[data-queryid]    
    changeAuthor: function ()   {
        document.querySelectorAll('input[name="authors"]').forEach(function (el) {
            el.addEventListener('change', function () {
                var href = window.location.href.replace(/&amp;/g, '&');
                const url = new URL(href);
                const params = url.searchParams;
                var authorId = this.checked ? $(this).val() : "";
                if (authorId) {
    ...
    changeLang: function (lang) {
        document.querySelectorAll('input[name="language"]').forEach(function (el) {
            el.addEventListener('change', function () {
                var href = window.location.href.replace(/&amp;/g, '&');
                const url = new URL(href);
                const params = url.searchParams;
                var lang = this.checked ? this.dataset.code : "";
                if (lang) {
    ...
    search: function () {
        document.querySelector('.menu_search')?.addEventListener('submit', function (e) {
            e.preventDefault();
            var params = new URLSearchParams();
            var idMselect;
            $(this).find("input,textarea,hidden,select").not("input[type='checkbox'], .date-range-picker, #searchin").each(function () {
                idMselect = $(this).attr("name");
                if ($(this).val() !== '' && $(this).val() !== 'Từ khóa tìm kiếm')
                    params.set(idMselect, $(this).val());
            });
            idMselect = $(this).attr("name");
            var keyword = $(this).find('[name="keyword"]').val();
            if ($(this).val() !== '' && keyword !== '' && typeof idMselect !== "undefined")
                params.set(idMselect, $(this).val());  
            Array.from(params.keys()).forEach(key => {
                if (!params.get(key)) params.delete(key);
            });
            window.location.href = `/ket-qua-tim-kiem?${DOM_Purify.sanitize(params.toString())}`;
            return false;
        });
    },
    tracking: function () {
        document.querySelectorAll('a[data-queryid]').forEach(function (el) {
            el.addEventListener('mousedown', function () {
                jQuery.ajax({
                    url: '/tracking',
                    type: 'POST',
                    contentType: 'application/json',
                    data: JSON.stringify({ QueryId: this.dataset.queryid }),
                    cache: false
                });
            });
        });
...
const DOM_Purify = {
    sanitize: function (html) {
        ...
        var forbiddenTags = ['script', 'iframe', 'object', 'embed', 'link', 'meta', 'base', 'form', 'style'];
        ...
        doc.querySelectorAll('*').forEach(function (el) {
            Array.from(el.attributes).forEach(function (attr) {
                ...
                // Xóa tất cả event handler (on*)
                if (name.startsWith('on')) {
                    el.removeAttribute(attr.name);
                    return;
                }

                // Xóa attribute có giá trị chứa protocol nguy hiểm
                if (/^\s*(javascript|data|vbscript)\s*:/i.test(val)) {
                    el.removeAttribute(attr.name);
                    return;
                }

                // Xóa style chứa expression hoặc url javascript
                if (name === 'style' && /(javascript|expression|vbscript)\s*[:(]/i.test(val)) {
                    el.removeAttribute(attr.name);
                    return;
                }
            });
...
// TrithuctreBooks monthly deals carousel demo
(function initMonthlyDealsCarousel() {
  function boot() {
    if (!window.jQuery || typeof jQuery.fn.owlCarousel !== "function") return;
    var $owl = jQuery(".ttb-monthly-products.owl-carousel");
    if (!$owl.length || $owl.data("owl.carousel")) return;
    $owl.owlCarousel({
      loop: true,
      nav: true,
      dots: true,
...
/* TrithuctreBooks partner and mobile account carousels */
(function initTrithuctreHomeCarousels() {
  function initialize() {
    if (!window.jQuery || typeof window.jQuery.fn.owlCarousel !== "function") {
      return;
    }

    var $partner = window.jQuery(".partner-box_section.key-partner .partner_list.owl-carousel");
    if ($partner.length && !$partner.data("owl.carousel")) {
      $partner.owlCarousel({
        loop: true,
        rewind: false,
        autoplay: true,
    ...
    var $upgrade = window.jQuery(".reading-pack-mb .ttb-upgrade-carousel.owl-carousel");
    if ($upgrade.length && !$upgrade.data("owl.carousel")) {
      $upgrade.owlCarousel({
        loop: true,
        rewind: false,
        autoplay: false,
...
/* TrithuctreBooks author and account carousel enforcement */
(function enforceTrithuctreCarousels() {
  function init() {
    if (!window.jQuery || typeof window.jQuery.fn.owlCarousel !== "function") {
      return;
    }
    ...
    function ensureCarousel($el, options) {
      if (!$el.length) {
        return;
      }
    ...
    ensureCarousel($(".tac-gia-tieu-bieu .ttb-author-carousel.owl-carousel"), {
      loop: true,
      rewind: false,
      autoplay: true,
      autoplayTimeout: 6000,
      ...
      responsive: {
        0: { items: 2, margin: 16 },
        576: { items: 3, margin: 18 },
        768: { items: 4, margin: 22 },
        1025: { items: 5, margin: 24 }
      }
    });

    ensureCarousel($(".reading-pack.section-dt .ttb-upgrade-desktop-carousel.owl-carousel"), {
      loop: true,
      rewind: false,
      autoplay: false,
      smartSpeed: 500,
    ...
    ensureCarousel($(".reading-pack-mb .ttb-upgrade-carousel.owl-carousel"), {
      loop: true,
      rewind: false,
      autoplay: false,
      smartSpeed: 500,
      ...
      margin: 16,
      responsive: {
        0: { items: 1.35, margin: 14 },
        576: { items: 2.15, margin: 16 },
